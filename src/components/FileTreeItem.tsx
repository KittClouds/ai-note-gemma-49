...
export function FileTreeItem({ item, level = 0 }: FileTreeItemProps) {
  ...
  console.log('FileTreeItem Debug - Rendering item:', {
    id: item.id,
    title: item.title,
    type: item.type,
    isSelected,
    currentSelectedId: state.selectedItemId
  });

  const handleClick = () => {
    console.log('FileTreeItem Debug - Click handler called for:', item.id, 'type:', item.type);
    
    if (isSelectionMode) {
      console.log('FileTreeItem Debug - In selection mode, toggling selection');
      toggleSelection(item.id);
    } else if (isFolder) {
      console.log('FileTreeItem Debug - Folder clicked, toggling');
      toggleFolder(item.id);
    } else {
      console.log('FileTreeItem Debug - Note clicked, selecting');
      selectItem(item.id);
    }
  };
