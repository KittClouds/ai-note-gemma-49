...
export class GraphRAG {
  ...
  constructor(dimension: number) {
    ...
    this.defineEdgeType('semantic', { symmetrical: false });
  ...
  addEdge(edge: GraphEdge): void {
    ...
    if (this.edgeSet.has(key)) return; // dedupe
    ...
    if (definition?.symmetrical) {
      ...
      const rKey = this.edgeKey(reverse);
      if (!this.edgeSet.has(rKey)) {
        this.edges.push(reverse);
        this.edgeSet.add(rKey);
      }
    }
  }
  ...
  clearEdges(type?: string): void {
    ...
    for (const e of this.edges) {
      if (e.type !== type) {
        kept.push(e);
        keptKeys.add(this.edgeKey(e));
      }
    }
    this.edges = kept;
    this.edgeSet = keptKeys;
  }

  getEdgeStats(): { total: number; byType: Record<string, number> } {
    const byType = new Map<string, number>();
    for (const e of this.edges) byType.set(e.type, (byType.get(e.type) || 0) + 1);
    return { total: this.edges.length, byType: Object.fromEntries(byType) };
  }
   ...
   * @param options.groupBy Optional metadata key to group nodes by before building sequences.
   */
  buildSequentialEdges({ metadataKey, groupBy }: { metadataKey: string; groupBy?: string }): void {
    ...
    for (const n of nodes) {
      const key = groupBy ? String(n.metadata![groupBy]) : '__all__';
      if (!groups.has(key)) groups.set(key, []);
      groups.get(key)!.push(n);
    }

    for (const group of groups.values()) {
      group.sort((a, b) => a.metadata![metadataKey] - b.metadata![metadataKey]);
      for (let i = 0; i < group.length - 1; i++) {
        this.addEdge({ source: group[i]!.id, target: group[i + 1]!.id, weight: 1.0, type: 'sequential' });
      }
    }
  }
