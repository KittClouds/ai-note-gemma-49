...
export const entityTemplates$ = queryDb(
  tables.entityTemplates.orderBy('createdAt', 'desc'),
  { label: 'entityTemplates$' }
);

export const templateAttributes$ = queryDb(
  tables.templateAttributes.orderBy('sortOrder', 'asc'),
  { label: 'templateAttributes$' }
);

// UI state queries with proper error handling and better reactivity
...
export const selectedItemId$ = computed((get) => {
  try {
    ...
    console.log('LiveStore Query Debug - Selected Item ID computed:', selectedId);
    return selectedId;
  } catch (error) {
...
export const expandedFolders$ = computed((get) => {
  try {
    ...
    console.log('LiveStore Query Debug - Expanded Folders computed:', expanded);
    return expanded;
  } catch (error) {
...
// Selected item query with error handling and better reactivity
export const selectedItem$ = computed((get) => {
  try {
    const selectedId = get(selectedItemId$);
    const allItems = get(allItems$);
    
    console.log('LiveStore Query Debug - Computing selected item for ID:', selectedId);
    
    if (!selectedId || !Array.isArray(allItems)) {
      console.log('LiveStore Query Debug - No selected ID or items not available');
      return null;
    }
    
    const foundItem = allItems.find(item => item.id === selectedId) || null;
    console.log('LiveStore Query Debug - Found selected item:', foundItem?.title || 'None');
...
export const createItemsByParentQuery = (parentId?: string) => computed((get) => {
  try {
    ...
    // FIXED: Proper root item filtering with better logging
    ...
    console.log('LiveStore Query Debug - Items by parent', parentId || 'root', ':', filtered.length, 'items');
    return filtered;
  } catch (error) {
