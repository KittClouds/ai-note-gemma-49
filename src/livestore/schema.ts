...
export const tables = {
  ...
  // NEW: QPS index snapshots table
  qpsIndexes: State.SQLite.table({
    name: 'qps_indexes',
    columns: {
      id: State.SQLite.text({ primaryKey: true }),
      version: State.SQLite.text(),
      checksum: State.SQLite.text(),
      data: State.SQLite.text(), // serialized QPS index
      metadata: State.SQLite.json({ schema: Schema.Any }),
      createdAt: State.SQLite.text(),
      updatedAt: State.SQLite.text()
    }
  ...
  entityTemplates: State.SQLite.table({
    ...
    columns: {
      ...
      isBuiltIn: State.SQLite.boolean({ default: false }),
  ...
  templateAttributes: State.SQLite.table({
    name: 'templateAttributes',
    columns: {
      id: State.SQLite.text({ primaryKey: true }),
      templateId: State.SQLite.text(),
      name: State.SQLite.text(),
      type: State.SQLite.text(),
      defaultValue: State.SQLite.json({ schema: Schema.Any, nullable: true }),
      unit: State.SQLite.text({ nullable: true }),
      required: State.SQLite.boolean({ default: false }),
...
export const events = {
  ...
  // NEW: QPS index events
  qpsIndexCreated: Events.synced({
    name: 'v1.QpsIndexCreated',
    schema: Schema.Struct({
      id: Schema.String,
      version: Schema.String,
      checksum: Schema.String,
  ...
  qpsIndexUpdated: Events.synced({
    name: 'v1.QpsIndexUpdated',
    schema: Schema.Struct({
      id: Schema.String,
      version: Schema.String,
      checksum: Schema.String,
  ...
  qpsIndexDeleted: Events.synced({
    name: 'v1.QpsIndexDeleted',
    schema: Schema.Struct({
      id: Schema.String
    })
  }),
  ...
  templateDeleted: Events.synced({
    name: 'v1.TemplateDeleted',
    schema: Schema.Struct({
      id: Schema.String
    })
  }),

  templateAttributeCreated: Events.synced({
    name: 'v1.TemplateAttributeCreated',
    schema: Schema.Struct({
      id: Schema.String,
  ...
  templateAttributeUpdated: Events.synced({
    name: 'v1.TemplateAttributeUpdated',
    schema: Schema.Struct({
      id: Schema.String,
      updates: Schema.Any,
      updatedAt: Schema.String
  ...
  templateAttributeDeleted: Events.synced({
    name: 'v1.TemplateAttributeDeleted',
    schema: Schema.Struct({
      id: Schema.String
    })
  }),
...
const materializers = State.SQLite.materializers(events, {
  ...
  // NEW: QPS index materializers
  'v1.QpsIndexCreated': ({ id, version, checksum, data, metadata, createdAt, updatedAt }) => {
    console.log('LiveStore Materializer Debug - QPS Index Created:', { id, version, checksum, createdAt });
    const result = tables.qpsIndexes.insert({ id, version, checksum, data, metadata, createdAt, updatedAt });
    console.log('LiveStore Materializer Debug - QPS index insert result:', result);
    return result;
  },

  'v1.QpsIndexUpdated': ({ id, version, checksum, data, metadata, updatedAt }) => {
    console.log('LiveStore Materializer Debug - QPS Index Updated:', { id, version, checksum, updatedAt });
    const result = tables.qpsIndexes.update({ version, checksum, data, metadata, updatedAt }).where({ id });
    console.log('LiveStore Materializer Debug - QPS index update result:', result);
    return result;
  },

  'v1.QpsIndexDeleted': ({ id }) => {
    console.log('LiveStore Materializer Debug - QPS Index Deleted:', { id });
    const result = tables.qpsIndexes.delete().where({ id });
    console.log('LiveStore Materializer Debug - QPS index delete result:', result);
    return result;
  },

  // Template management materializers
  'v1.TemplateCreated': ({ id, name, kind, description, color, icon, isBuiltIn, category, createdAt, updatedAt }) => {
    console.log('LiveStore Materializer Debug - Template Created:', { id, name, kind });
    const result = tables.entityTemplates.insert({ 
      id, name, kind, description, color, icon, isBuiltIn, category, createdAt, updatedAt 
    });
    console.log('LiveStore Materializer Debug - Template insert result:', result);
    return result;
  },

  'v1.TemplateUpdated': ({ id, updates, updatedAt }) => {
    console.log('LiveStore Materializer Debug - Template Updated:', { id, updates, updatedAt });
    const result = tables.entityTemplates.update({ ...updates, updatedAt }).where({ id });
    console.log('LiveStore Materializer Debug - Template update result:', result);
    return result;
  },

  'v1.TemplateDeleted': ({ id }) => {
    console.log('LiveStore Materializer Debug - Template Deleted:', { id });
    const operations = [
      tables.templateAttributes.delete().where({ templateId: id }),
      tables.entityTemplates.delete().where({ id })
    ];
    console.log('LiveStore Materializer Debug - Template delete result:', operations);
    return operations;
  },

  'v1.TemplateAttributeCreated': ({ id, templateId, name, type, defaultValue, unit, required, sortOrder, category, createdAt, updatedAt }) => {
    console.log('LiveStore Materializer Debug - Template Attribute Created:', { id, templateId, name, type });
    const result = tables.templateAttributes.insert({ 
      id, templateId, name, type, defaultValue, unit, required, sortOrder, category, createdAt, updatedAt 
    });
    console.log('LiveStore Materializer Debug - Template attribute insert result:', result);
    return result;
  },

  'v1.TemplateAttributeUpdated': ({ id, updates, updatedAt }) => {
    console.log('LiveStore Materializer Debug - Template Attribute Updated:', { id, updates, updatedAt });
    const result = tables.templateAttributes.update({ ...updates, updatedAt }).where({ id });
    console.log('LiveStore Materializer Debug - Template attribute update result:', result);
    return result;
  },

  'v1.TemplateAttributeDeleted': ({ id }) => {
    console.log('LiveStore Materializer Debug - Template Attribute Deleted:', { id });
    const result = tables.templateAttributes.delete().where({ id });
    console.log('LiveStore Materializer Debug - Template attribute delete result:', result);
    return result;
  }
});
